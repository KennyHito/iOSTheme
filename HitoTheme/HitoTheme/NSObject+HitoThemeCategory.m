...
#import "NSObject+HitoThemeCategory.h"

#define HitoTHEME_COLOR_ARGS_KEY @"HitoTHEME_COLOR_ARGS_KEY"
#define HitoTHEME_IMAGE_ARGS_KEY @"HitoTHEME_IMAGE_ARGS_KEY"

@implementation NSObject (HitoThemeCategory)
/** 主题颜色池*/
...
- (id)Hito_performSelector:(SEL)selector withObjects:(const NSArray<id> *)objects {
    ...
    for (int i = 0; i < count; i++) {
        ...
        id obj = objects[i];
        // 如果是主题颜色参数颜色，则设置
        if ([obj isKindOfClass:[NSString class]] && [obj isEqualToString:HitoTHEME_THEME_COLOR]) {
        ...
        if ([argumentTypeString isEqualToString:@"@"]) { // id
            ...
            if ([obj isKindOfClass:[NSDictionary class]]) { // NSDictionary
                ...
                for (NSString *key in keys) {
                    ...
                    if ([value isKindOfClass:[NSString class]] && [value isEqualToString:HitoTHEME_THEME_COLOR]) {
                        // 替换成颜色
...
- (void)Hito_addToThemeColorPoolWithSelector:(SEL)selector objects:(NSArray<id> *)objects {
    ...
    [mapTable setObject:objects forKey:HitoTHEME_COLOR_ARGS_KEY];
    // 判断是否已经在主题色池中
    ...
    if (_currentThemeColor) { // 已经设置主题色，直接设置
        [self Hito_performSelector:selector withObjects:objects];
    }
}
...
- (void)Hito_removeFromThemeColorPoolWithSelector:(SEL)selector {
    ...
    for (NSMapTable *subMapTable in [[self themeColorPool] copy]) {
        ...
        while (key = [enumerator nextObject]) {
            if (![key isEqualToString:HitoTHEME_COLOR_ARGS_KEY]) {
                objectKey = key;
                break;
            }
        }
...
- (void)Hito_setThemeColor:(UIColor *)color
{
    ...
    for (NSMapTable *mapTable in [_themeColorPool copy]) {
        ...
        if ([objectKey containsString:@":"]) { // 方法
            // 取出参数
            NSArray *args = [mapTable objectForKey:HitoTHEME_COLOR_ARGS_KEY];
            // 取出方法
            NSString *selectorName = [objectKey substringFromIndex:[[NSString stringWithFormat:@"%p", object] length]];
            SEL selector = NSSelectorFromString(selectorName);
            // 调用方法，设置属性
            [object Hito_performSelector:selector withObjects:args];
        } else { // 成员属性
            // 取出属性值
...
- (void)Hito_reloadThemeImageWithThemeColor:(UIColor *)themeColor setting:(HitoThemeImageSettingBlock)block {
    if (themeColor) { // 有主题色，设置主题色
        [self Hito_setThemeColor:themeColor];
    }
